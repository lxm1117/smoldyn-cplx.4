...
cmdssptr scmdssalloc(enum CMDcode (*cmdfn)(void*,cmdptr,char*),void *cmdfnarg,const char *root) {
	...
	cmds->cmd=NULL;
	cmds->cmdi=NULL;
	cmds->cmdfn=cmdfn;				// check sim.c &docommand()
...
int scmdstr2cmd(cmdssptr cmds,char *line2,double tmin,double tmax,double dt) {
	...
	if(strchr("ba@ix",ch)) {
		...
		if(ch=='b') cmd->on=cmd->off=tmin-dt;
			...
			itct=sscanf(line2,"%lg",&cmd->on);
			if(itct!=1) return 3;
			cmd->off=cmd->on;
			if(!(line2=strnword(line2,2))) return 4; }
		else if(ch=='i') {
			itct=sscanf(line2,"%lg %lg %lg",&cmd->on,&cmd->off,&cmd->dt);
			if(itct!=3) return 3;
			if(cmd->on<tmin) cmd->on=tmin;
			if(cmd->off>tmax) cmd->off=tmax;
			if(cmd->dt<=0) return 5;
			...
			itct=sscanf(line2,"%lg %lg %lg %lg",&cmd->on,&cmd->off,&cmd->dt,&cmd->xt);
			if(itct!=4) return 3;
			if(cmd->on<tmin) cmd->on=tmin;
			if(cmd->off>tmax) cmd->off=tmax;
			if(cmd->dt<=0) return 5;
		...
		if(ch=='B') cmd->oni=cmd->offi=-1;
			...
			if(itct!=1) return 3;
			if(!(line2=strnword(line2,2))) return 4;
			itct=sscanf(line2,Q_LLI,&cmd->offi);
			if(itct!=1) return 3;
			if(!(line2=strnword(line2,2))) return 4;
	...
	if(cmd->str[strlen(cmd->str)-1]=='\n')
		cmd->str[strlen(cmd->str)-1]='\0';
	// printf("command.c line 326, cmd->str=%s\n", cmd->str);
	return 0; }
...
enum CMDcode scmdexecute(cmdssptr cmds,double time,double simdt,Q_LONGLONG iter,int donow) {
	enum CMDcode code1,code2;			// check smolcmd.c
	cmdptr cmd;
	double dt;
	void *voidptr;
	...
	if(cmds->cmdi)						// integer execution times
		while((q_length(cmds->cmdi)>0) && (q_frontkeyL(cmds->cmdi)<=iter || donow)) {
			...
			cmd=(cmdptr)voidptr;
			cmd->invoke++;
			// printf("command.c line 359, time=%f, cmd->str=%s\n", time, cmd->str);
			code1=(*cmds->cmdfn)(cmds->cmdfnarg,cmd,cmd->str);			// cmd->cmdfn is docommand()
			if(code1==CMDwarn) {
	...
	if(cmds->cmd)				// float execution times
		while((q_length(cmds->cmd)>0) && (q_frontkeyD(cmds->cmd)<=time || donow)) {
			...
			cmd=(cmdptr)voidptr;
			cmd->invoke++;
			// printf("command.c line 376, time=%f, cmd->str=%s\n", time, cmd->str);
