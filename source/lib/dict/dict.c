...
DictInsert(Dict d, const char *key, const char *value)
{
	...
	if(DictEltExist(d, key, value)) return;
...
/* test to delete one specific element */
int DictDeleteElt(Dict d, const char *key, const char *value){
	struct elt *e;
	struct elt *e_tmp;
	
	if(DictEltExist(d, key, value)){
		for(e=d->table[hash_function(key) % d->size]; e!=0; e=e->next){
			if(!strcmp(e->next->key, key) && !strcmp(e->next->value, value)){
				e_tmp=e->next->next;
				e->next=e_tmp;
				free(e->next);	
			}
...
int DictEltExist(Dict d, const char *key, const char *value){
	struct elt *e;

	for(e=d->table[hash_function(key) % d->size]; e!=0; e=e->next){
		if(!strcmp(e->key,key) && !strcmp(e->value, value)){			// stcmp(e->key,key) is redundant
			return 1;
		}
	}
	return 0;
