...
int latticeexpandspecies(latticeptr lattice,int maxspecies) {
	int i;
	int *newspecies_index,*newmaxmols,*newnmols;
...
void latticeoutput(simptr sim) {
	...
	for(lat=0;lat<latticess->nlattice;lat++) {
		...
		for(i=0;i<lattice->nspecies;++i) {
			simLog(sim,2,"   %s has %i molecules\n",sim->mols->spname[lattice->species_index[i]],lattice->nmols[i]); }

		if(lattice->nsv) {
			NSV_CALL(nsv_print(lattice->nsv,buffer));
			simLog(sim,2,"  External nsv class description: %s",buffer); }
		if(lattice->pde) {
...
int checklatticeparams(simptr sim,int *warnptr) {
	int error,warn,lat,d,i,interact,s;
	double volume;
	latticessptr latticess;
	latticeptr lattice;
	char string[STRCHAR];
	...
	for(lat=0;lat<latticess->nlattice;lat++) {
      ...
      warn++;simLog(sim,7," WARNING: there is no interface port assigned to lattice %s\n",lattice->latticename); }
			...
			if(lattice->port->srf->npanel[PStri]>0) {warn++;simLog(sim,5,"WARNING: porting surface contains triangle panels which will be ignored"); }
			if(lattice->port->srf->npanel[PSsph]>0) {warn++;simLog(sim,5,"WARNING: porting surface contains sphere panels which will be ignored"); }
			if(lattice->port->srf->npanel[PScyl]>0) {warn++;simLog(sim,5,"WARNING: porting surface contains cylinder panels which will be ignored"); }
			if(lattice->port->srf->npanel[PShemi]>0) {warn++;simLog(sim,5,"WARNING: porting surface contains hemisphere panels which will be ignored"); }
			if(lattice->port->srf->npanel[PSdisk]>0) {warn++;simLog(sim,5,"WARNING: porting surface contains disk panels which will be ignored"); }}
...
/* latticeaddspecies */
int latticeaddspecies(latticeptr lattice,int species_index) {
	int i,er;

	for(i=0;i<lattice->nspecies;++i)
		if(lattice->species_index[i]==species_index)
			return 2;
	...
	i=lattice->nspecies;
	lattice->species_index[i]=species_index;
	lattice->nmols[i]=0;
	lattice->nspecies++;
...
int latticeaddmols(latticeptr lattice,int nmol,int i,double *poslo,double *poshi,int dim) {
	...
	if(j==lattice->nspecies) {
		er=latticeaddspecies(lattice,i);
		if(er) return 1; }

	if(lattice->nmols[j]==lattice->maxmols[j]) {
...
/* latticeaddport */
int latticeaddport(latticeptr lattice,portptr port) {
	lattice->port = port;
	latticesetcondition(lattice->latticess,SCparams,0);
	return 0; }
...
latticeptr latticereadstring(simptr sim,ParseFilePtr pfp,latticeptr lattice,const char *word,char *line2) {
	char nm[STRCHAR],ch;
	int axis,s,itct,i,j,nmol,d,dim,move,lat,er;
	...
	if(!strcmp(word,"name")) {                        // name
      ...
      lat=stringfind(sim->latticess->latticenames,sim->latticess->nlattice,nm);
		...
		itct=sscanf(line2,"%i %lg %lg",&axis,&min,&max);
    ...
    CHECKS(!line2,"unexpected text following lengthscale"); }
		...
		CHECKS(lattice,"lattice name has to be entered before port");
    CHECKS(sim->portss,"ports need to be defined before being assigned");
		...
		er=latticeaddport(lattice,sim->portss->portlist[s]);
		CHECKBUG(er==0,"SMOLDYN BUG adding port to lattice");
		CHECKS(!strnword(line2,2),"unexpected text following port"); }
		...
		while(line2) {
			...
			if(!strcmp(nm,"all")) {
				...
				s=stringfind(sim->mols->spname,sim->mols->nspecies,nm);
				CHECKS(s>0,"species '%s' not recognized",nm);
				er=latticeaddspecies(lattice,s);
				CHECKS(er!=1,"out of memory adding species to lattice"); }
			line2=strnword(line2,2); }
...
int latticesupdateparams(simptr sim) {
	...
	for(i=0;i<sim->latticess->nlattice;++i) {
		...
		if(port) {											// send port information to NSV code
			for(j=0;j<port->srf->npanel[PSrect];++j) {
				...
				switch(port->srf->panels[PSrect][j]->npts) {
				case 1:											// 1D system
					start[0]=panel->point[0][0];
					end[0]=panel->point[0][0];
					norm[0]=panel->front[0];
					break;
				case 2:											// 2D system
					start[0]=panel->point[0][0];
					...
					norm[(int)(panel->front[1])]=panel->front[0];
					break;
				case 4:											// 3D system
					start[0]=panel->point[0][0];
					start[1]=panel->point[0][1];
				...
				if(lattice->type==LATTICEnsv) {
					NSV_CALL(nsv_add_interface(lattice->nsv,sim->dt,start,end,norm,sim->dim)); }
		...
		}} // disable reactions in particle space if needed
