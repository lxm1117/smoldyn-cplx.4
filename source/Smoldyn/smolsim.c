...
simptr simalloc(const char *fileroot) {
  ...
  int order,k;
	...
	sim->tbreak=DBL_MAX;
	sim->dt=1;
	for(k=0;k<MAXORDER;k++) sim->rxnss[k]=NULL;
	sim->mols=NULL;
	sim->wlist=NULL;
	sim->srfss=NULL;
	sim->boxs=NULL;
	...
	CHECKMEM(sim->cmds=(void*) scmdssalloc(&docommand,(void*)sim,fileroot)); 	// docommand in smolcmd.c
	return sim;
...
void simfree(simptr sim) {
	int dim,order,molec_num,maxsrf, k;
...
void simsystemoutput(simptr sim) {
	...
	for(order=0;order<MAXORDER;order++){
		if(sim->rxnss[order])	rxnoutput(sim,order);
	}
	compartoutput(sim);
	portoutput(sim);
	// bngoutput(sim);
...
int simreadstring(simptr sim,ParseFilePtr pfp,const char *word,char *line2) {
	...
	char species_name[STRCHAR], site_name[STRCHAR], cname[STRCHAR];
	int sitecode;
	int er,dim,i,j,nmol,d,i1,s,c,ll,order,molec_num,nprod,*index;// more;
	int sunit; //complex
	int r,ord,prd,itct,prt,lt,f;
	...
	// molecules
		...
		if(line2){
			if(strstr(line2,"<v>")){
				itct=sscanf(line2,"%s %s",nm,fname);
				sim->vfile=EmptyString();
				strcpy(sim->vfile,strextract(fname,"''"));
				printf("sim->vfile:%s\n",sim->vfile);
	...
	else if(!strcmp(word,"sites")){							// cplx
		sscanf(line2,"%s",species_name);					
		line2=strnword(line2,2);
		sitecode=0;
		while(line2!=NULL){
			sscanf(line2,"%s",site_name);	
			er=moladdsites(sim,species_name,site_name,sitecode);
			CHECKS(er!=-1, "failed to allocate memory");
			CHECKS(er!=-2, "incorrect species name");
			line2=strnword(line2,2);
			sitecode++;
		}		
	}

	else if(!strcmp(word,"dif_sites")){
		sscanf(line2,"%s",species_name);
		line2=strnword(line2,2);

		while(line2!=NULL){
			sscanf(line2, "%s", site_name);
			er=moldifsites(sim,species_name,site_name);	
			CHECKS(er!=-1, "incorrect species name");
			CHECKS(er!=-2, "incorrect site name");
			line2=strnword(line2,2);
		}
	}
		...
		*/
		...
		CHECKS(er!=-1,"difc format: name[(state)] value");
		...
		//complex
		itct=sscanf(line2,"%d %d",&num_mol,&sunit);	
		...
		i=readmolname_cplx(sim,line2,&ident,&sites);
		...
		er=addmol_cplx(sim,num_mol,sunit,i,ident,sites,poslo,poshi,0,1,1);
		...
		if(line2) {
			...
			er=addsurfmol_cplx(sim,num_mol,sunit,i,ident,sites,ms,v1,NULL,s,ps,nm1);
			...
			er=addsurfmol_cplx(sim,num_mol,sunit,i,ident,sites,ms,NULL,NULL,s,ps,nm1);	
			CHECKS(er!=1,"in surface_mol, unable to allocate temporary storage space");
		...
		itct=sscanf(line2,"%d %d %s",&num_mol,&sunit,nm);
		...
		molsetlistlookup(sim,0,index,MSall,ll);
	...
	// about runtime commands
	else if(!strcmp(word,"multibinding")){
		if(!strcmp(line2,"on"))
			sim->multibinding=1;
		else sim->multibinding=0;
	}
	else if(!strcmp(word, "events")){			// record reaction events, cplx
		sim->events=fopen(line2,"w");	
	}
		...
		er=scmdstr2cmd((cmdssptr) sim->cmds,line2,sim->tmin,sim->tmax,sim->dt);				// in SimCommand.c
	...
	// reactions
		...
		// removing repeated key entries in rxnss->entrylist; circulate the linkedlist 
		if(!strcmp(line2,"end")){
			GSList *s_entry,*s_last,*s_list;
			for(s=0;s<MAXORDER;s++){
				if(sim->rxnss[s]){
					s_list=(GSList*)g_hash_table_get_keys(sim->rxnss[s]->entrylist);
					while(s_list){
						s_entry=(GSList*)s_list->data;
						s_last=g_slist_last(s_entry);
						s_last->next=s_entry;
						s_list=s_list->next;
					}
		...
		*/
		...
		prd1=NULL;
		prd2=NULL;
		rxnrate=rxnparser(sim->mols,line2,&molec_num,&order,&nprod,&rct1,&rct2,&prd1,&prd2); 
		printf("rname:%s, rxnrate=%f\n", rname, rxnrate);
		if(!strstr(rname,"<v>"))
			CHECKS(rxnrate!=NULL, "rxnparse error, %s", rname);
		if(rxnrate) {
			RxnAddReaction_cplx(sim,rname,molec_num,order,nprod,rct1,rct2,prd1,prd2,cmpt,srf,rxnrate);
			line2=strnword(line2,2);
	...
	*/
		...
		if(!strcmp(nm,"all")) {
			...
			line2=strnword(line2,3);
			lilist=List_ReadStringLI(line2);
		...
		if(rpart==RPirrev);
			...
			// for(prd=0;prd,rxn->nprod && rxn->ident[prd]!=i;prd++);
			for(prd=0;prd,rxn->nprod;prd++){
				if(rxn->prd[prd]->ident==i) break;
			}
...
int simupdate(simptr sim) {
	...
	// printf("after boxesupdate, time=%f\n", sim->time);

	er=molsort(sim,0);
	CHECK(er!=1);
	// printf("after molsort, time=%f\n", sim->time);

	if(sim->condition==SCinit && sim->cmptss)
		simLog(sim,2," setting up compartments\n");
	er=compartsupdate(sim);
	CHECK(er!=1);
	// printf("after compartsupdate, time=%f\n", sim->time);
...
int simdocommands(simptr sim) {
	...
	ccode=scmdexecute((cmdssptr) sim->cmds,sim->time,sim->dt,-1,0);			// scmdexecute() in SimCommand.c, iter=-1
	er=simupdate(sim);
	if(er) return 8;
...
int simulatetimestep(simptr sim) {
	...
	if(sim->srfss) {														// deal with surface or wall collisions
		for(ll=0;ll<sim->srfss->nmollist;ll++) {
			if(sim->srfss->srfmollist[ll] & SMLdiffuse && sim->mols->nl[ll]) {
		    	er=(*sim->surfacecollisionsfn)(sim,ll,0);		
				if(er){
					if(sim->events)
						fprintf(sim->events,"sim timestep, ll=%d, er=%d\n", ll, er);	
		}}}	// check all molecules
		for(ll=0;ll<sim->srfss->nmollist;ll++)				// surface-bound molecule actions
			if(sim->srfss->srfmollist[ll] & SMLsrfbound && sim->mols->nl[ll])
				(*sim->surfaceboundfn)(sim,ll); }
	else {
		if(sim->mols)
			for(ll=0;ll<sim->mols->nlist;ll++)
				if(sim->mols->diffuselist[ll] && sim->mols->nl[ll])
	...
	*/
	if(sim->srfss) {												
		for(ll=0;ll<sim->srfss->nmollist;ll++) {
			if(sim->srfss->srfmollist[ll] & SMLdiffuse && sim->mols->nl[ll]) {
		    	//er=(*sim->surfacecollisionsfn)(sim,ll,1);			
		    	er=(*sim->surfacecollisionsfn)(sim,ll,0);			
				if(er){
					if(sim->events)
						fprintf(sim->events,"sim timestep, ll=%d, er=%d\n", ll, er);	
	...
	if(er)
		return 29;
	er=(*sim->assignmols2boxesfn)(sim,0,0);					// assign again (all, reborn)
	if(er) return 2;
	/*
	er=filDynamics(sim);
	if(er) return 11;
...
int smolsimulate(simptr sim) {
	...
	if(sim->events)	
		fprintf(sim->events,"smolsim.c 1949, er=%d\n",er);
	sim->elapsedtime+=difftime(time(NULL),sim->clockstt);
	// at the end of simulation, record all ca molecules' positions
	if(sim->events){
		for(ll=0;ll<sim->mols->nlist;ll++){
			nmol=sim->mols->nl[ll];
			...
			for(m=0;m<nmol;m++){
				mptr=mlist[m];
				fprintf(sim->events, "rxn_time=%f end mptr->serno=%d pos[0]=%f pos[1]=%f pos[2]=%f complex_id=%d\n", sim->time, mptr->serno, mptr->pos[0], mptr->pos[1], mptr->pos[2], mptr->complex_id); 			
				if(mptr->ident==1 && mptr->sites_val==1) fprintf(sim->events, "ca[0]:serno=%d\n", mptr->sites[0]->bind->serno); 
	}}}
