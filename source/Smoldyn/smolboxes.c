...
int boxaddmol(moleculeptr mptr,int ll) {
	...
	if(bptr->nmol[ll]==bptr->maxmol[ll]){
		if(expandbox(bptr,bptr->maxmol[ll]+1,ll)){ 
			printf("bptr->nmol[ll]=%d, bptr->maxmol[ll]=%d, mptr->ident=%d\n", bptr->nmol[ll],bptr->maxmol[ll],mptr->ident);
			return 1;
	}}
	
	bptr->mol[ll][bptr->nmol[ll]++]=mptr;	// i.e., bptr->mol[ll][bptr->nmol[ll]]=mptr; bptr->nmol[ll]++;
...
void boxremovemol(moleculeptr mptr,int ll) {
	...
	bptr->mol[ll][m]=bptr->mol[ll][--bptr->nmol[ll]];			// i.e., bptr->nmol[ll]--; bptr->mol[ll][m]=bptr->mol[ll][bptr->nmol[ll]];
	mptr->box=NULL;
	return; }
...
int expandbox(boxptr bptr,int n,int ll) {
	...
	if(maxmol>0) {
		...
		maxmol=0;
		mlist=NULL; }
	if(bptr->mol[ll])	free(bptr->mol[ll]);
	bptr->mol[ll]=mlist;
...
int boxesupdatelists(simptr sim) {
	...
	if(mpbox>0) {
		...
		flt2=(double)molcount(sim,-5,NULL,MSall,NULL,-1);
...
int reassignmolecs(simptr sim,int diffusing,int reborn) {
	...
	// for(ll=0;ll<sim->mols->nlist;ll++)			// don't think need to reassign immobile molecules
	for(ll=0;ll<1;ll++)
		if(sim->mols->listtype[ll]==MLTsystem)
			if(diffusing==0 || sim->mols->diffuselist[ll]==1){
				...
				if(!reborn) m=0;
				else m=sim->mols->topl[ll];				// or sim->mols->nl[ll]==sim->mols->topl[ll]
				for(;m<nmol;m++) {
					mptr=mlist[m];
