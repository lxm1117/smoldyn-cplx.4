		...
		if(new_mol==-1){						
			...
			if(order==2){					// a+b->c 	//no need to kill mptr2, but mptr2 and mptr1 possess the same physical location	
				...
				if(mptr1->tot_sunit>1 && mptr2->tot_sunit>1){	
					...
					if(mptr1->ident==7 && (mptr2->ident==4 || mptr2->ident==3) && mptr1->dif_molec!=NULL){
						difmolec(sim,mptr1,&dif_molec1);
						difmolec(sim,mptr2,&dif_molec2);
						printf("sim->time=%f dif_molec1->serno=%d dif_molec2->serno=%d\n", sim->time, dif_molec1->serno, dif_molec2->serno);
					}
				...
				for(ll2=ll1;ll2<sim->mols->nlist;ll2++){
					...
					for(m2=0;m2<nmol2;m2++) {
						...
						if(mptr1->pos==mptr2->pos){
							...
							if(!r) r=bireact_test(sim,1,mptr2,mptr1,&list_len,NULL,NULL,NULL);
							if(!r) continue;
	...
	site_cplx:if(cplx_tmp){
			if(cplx_tmp->dif_bind==mptr){	
				...
				if(mptr->dif_molec) 
					...
					cplx_tmp->dif_bind_site=-1; cplx_tmp->dif_bind=NULL; cplx_tmp->dif_molec=NULL;}
			}
		for(s=1,mptr_tmp=mptr->to;s<mptr->tot_sunit;s++,mptr_tmp=mptr_tmp->to){
			mptr_tmp->dif_molec=cplx_tmp->dif_molec;
		}			
		if(mols->sim->events) fprintf(mols->sim->events,"react.c 3285 mptr->complex_id=%d mptr->dif_molec==NULL:%d\n", mptr->complex_id, mptr->dif_molec==NULL);
	}
	return;
}
...
int syncpos(molssptr mols, moleculeptr mptr, int site, double *offset){
	...
	for(k=0;k<mols->spsites_num[mptr->ident];k++){
		if(mptr->sites[k]->bind) {
			for(k0=k-1;k0>=0 && mptr->sites[k0]->bind;k0--){
				if(mptr->sites[k0]->bind==mptr->sites[k]->bind){
					printf("binding err, serno=%d binds to the same molecule serno=%d at k=%d and k1=%d\n", mptr->serno, mptr->sites[k]->bind->serno, k, k1);	
					return -1;
			}}
			if(k!=site) {
				if(mptr->sites[k]->bind->pos!=mptr->pos) mptr->sites[k]->bind->pos=mptr->pos;
			}
			else{
				for(d=0;d<mols->sim->dim;d++) mptr->sites[k]->bind->pos[d]=mptr->pos[d];
			}	
			for(k1=0;;k1++){
				if(mptr->sites[k]->bind->sites[k1]->bind==NULL) continue;
				if(mptr->sites[k]->bind->sites[k1]->bind==mptr || k1>=mols->spsites_num[mptr->sites[k]->bind->ident]) break;
			}
			if(k1<mols->spsites_num[mptr->sites[k]->bind->ident]){ 
				if(mptr->sites[k]->bind->sites[k1]->bind!=mptr){ 
					printf("binding err, mptr->serno=%d, mptr->ident=%d, mptr:bind[%d]->serno=%d, mptr:bind[%d]->ident=%d, k1=%d \n", mptr->serno, mptr->ident, k, mptr->sites[k]->bind->serno, k, mptr->sites[k]->bind->ident,k1);
					return -1;
				}
				else if(k!=site) { 
					syncpos(mols,mptr->sites[k]->bind,k1,NULL);			// if fill in the offset, this may cause double updating the complex
				}
			}
			if(offset){
				if(k!=site){
					if(mptr->sites[k]->bind->tot_sunit>1){
						if(complex_pos(mols->sim,mptr->sites[k]->bind,"pos_line3218",offset,1)==-1) return -1;
			}}}
	}}
	return 0;
}
...
int difmolec(simptr sim,moleculeptr mptr,moleculeptr *dif_molecptr){
	...
	if(mptr->complex_id==-1){
		...
		if(cplx_dif){
			...
			//if(dif_molec->sites[dif_molec->dif_site]->site_type==0){
			//	return dif_molec->sites[dif_molec->dif_site]->bind_id;		
			//}
		}
		else{
