	...
	bool lineXsurface(const Kairos::Vect3d& p1, const Kairos::Vect3d& p2) const {
		...
		for (int i = 0; i < PSMAX; ++i) {
			for (int j = 0; j < s->npanel[i]; ++j) {
				if (lineXpanel(p1t,p2t,s->panels[i][j],s->srfss->sim->dim,crsspt,&face1,&face2,NULL,NULL,NULL)==1) {
...
void nsv_add_reaction(NextSubvolumeMethod* nsv,rxnstruct *reaction) {
	...
	const int *reactant_ids = reaction->ident;	// reaction->rctident;
	const int nproducts = reaction->nprod;
	const int *product_ids = reaction->ident; // reaction->prdident;
		...
		for (int i = 0; i < lattice->nsurfaces; ++i) {
			...
			for (int j = 0; j < PSMAX; ++j) {
				for (int k = 0; k < lattice->surfacelist[i]->npanel[j]; ++k) {
					if (lineXpanel(m->posx,cc.data(),lattice->surfacelist[i]->panels[j][k],port->portss->sim->dim,crsspt,&face1,&face2,NULL,NULL,NULL)==1) {
	...
	for (int i = 0; i < ns; ++i) {
		...
		for (int j = 0; j < np; ++j) {
			...
			for (int k = 0; k < port->srf->npanel[PSrect]; ++k) {
				if (lineXpanel(s->particlesx[j].data(),s->particles[j].data(),port->srf->panels[PSrect][k],sim->dim,crsspt,&face1,&face2,NULL,NULL,NULL)) {
...
//				if (positions[nout][1] >= 10) std::cout << "x >= -10: "<<positions[nout]<<std::endl;
	...
	if (nout > 0) {
		er = portputmols2(sim,port,nout,species[0],species,positions,positionsx);
